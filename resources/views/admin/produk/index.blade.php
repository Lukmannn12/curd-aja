                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="GET" class="mb-4 flex flex-wrap gap-2 items-center">
                        ...
                        <select name="filter_stok" class="rounded border-gray-300 pr-10 py-2 text-xs">
                            ...
                            <option value="hampir" {{ request('filter_stok')=='hampir' ? 'selected' : '' }}>Stok < 10</option>
                            <option value="tersedia" {{ request('filter_stok')=='tersedia' ? 'selected' : '' }}>Stok Tersedia</option>
                        </select>

                        <select name="perPage" class="rounded border-gray-300 pr-10 py-2 text-xs">
                            <option value="5" {{ request('perPage') == 5 ? 'selected' : '' }}>5 Data</option>
                            <option value="10" {{ request('perPage') == 10 ? 'selected' : '' }}>10 Data</option>
                        </select>

                        <button type="submit"
                            class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 text-xs">
                            Terapkan
                        </button>
                    </form>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-sm text-gray-800">
                                @forelse ($produks as $produk)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-3">{{ ($produks->currentPage() - 1) * $produks->perPage() + $loop->iteration }}</td>
